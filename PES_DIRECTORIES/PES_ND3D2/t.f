      real*8 function tnormed(p1,q1,p2,p,th,ph,thp,php)
c
...
c
      implicit none
      integer p1,q1,p2,p
      real*8 t, th,ph,thp,php, xn, Pi, dq1, one, two
...
!      write(6,*) p1,q1,p2,p,th,ph,thp,php
      ...
      real*8 function tprime(p1,q1,p2,p,th,ph,thp,php)
c
...
!      write(6,*) p1,q1,p2,p,th,ph,thp,php
     ...
     $     * (4*Pi) / sqrt(xn)
      ...
      real*8 function t(p1,q1,p2,p,th,ph,thp,php)
      implicit none
      integer p1,q1,p2,p, r1,r2,r
      real*8 th,ph,thp,php, d1,d2,fact, PLMx,
...
! obeying painfully to SG conventions...
     ...
     $           * cos((r2*php+r*ph)*pi/180.0d0) / (2*PI)
      ...
      real*8 FUNCTION PLM(LIN,MIN,COSTH)
C
...
C          FOR MIN.LT.0 THIS VALUE SHOULD BE MULTIPLIED BY PARIT(MIN)
C
C     real*8 FUNCTION PM1(LIN,MIN,COSTH)
...
9000  PLM=PLM*SQRT(XL+0.5D0)
      ...
      real*8 FUNCTION THRJ(F1,F2,F3,G1,G2,G3)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C     SMALL CHANGES 31 JUL 95 (SG)
  ...
  601   FORMAT(' *** DIMENSION ERROR IN THRJ - INDEX.GT.MXIX',2I5)
   ...
   20 INT=ABS(F1-F2-G3)+0.0001D0
      ...
      real*8 FUNCTION PARIT(I)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      PARIT=1.D0
      IF((I/2)*2-I.NE.0) PARIT=-1.D0
      RETURN
      END
